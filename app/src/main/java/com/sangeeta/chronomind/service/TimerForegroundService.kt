...
class TimerForegroundService : Service() {
    ...
    private fun startTicking() {
        ...
        tickJob = serviceScope.launch {
            while (isActive) {
                ...
                val newElapsed = running.elapsedSeconds + 1L
                activityRepo.updateTimer(running.id, newElapsed, running = true)
                updateNotification(formatTime(newElapsed))

                val isAutoFinishTimer =
                    running.targetType != "STOPWATCH" && running.completionStyle == "TIMEREND"
                val targetSeconds = running.targetMinutes * 60L

                if (isAutoFinishTimer && targetSeconds > 0L && newElapsed >= targetSeconds) {
                    activityRepo.completeSession(running, finalElapsed = newElapsed)
                    stopForeground(STOP_FOREGROUND_REMOVE)
                    stopSelf()
                    break
                }
    ...
    private fun pauseTicking() {
        ...
        serviceScope.launch {
            val running = activityRepo.observeRunning().firstOrNull()
            if (running != null) {
                val latestRunning = running.copy(elapsedSeconds = running.elapsedSeconds)
                activityRepo.pauseSession(latestRunning)
            }
